...
function LecturerEdit() {
    ...
    let liName=["li1","li2","li3","li4"];//for change background color og current component in navbar
                        ...
                        axios.post('http://localhost:5000/lecturer/token', { refreshToken: token.refreshToken })
                            .then(res => {
                                if (res.data.accessToken !== "") {//if success generate a new access token
                                    ...
                                    window.location = '/lecturer/editMe';
                                } else {
                                    deleteCookieAndGoLogin();
                                }
                            })
    ...
    function getLecturerData(idParam) {
        axios.get('http://localhost:5000/lecturer/getLecturerInfo', { params: { id: idParam } })
            .then(res => {
                dqs('fname').value = res.data.lecturer.firstname;
                dqs('lname').value = res.data.lecturer.lastname;
                dqs('email').value = res.data.lecturer.email;
                dqs('idd').value = res.data.lecturer.id;
                dqs(`${res.data.lecturer.gender}`).checked = "true"
                setGender(res.data.lecturer.gender);
                console.log(res.data.lecturer.pic);
                dqs('imgId').src = `/studentImages/${res.data.lecturer.pic}`;
            })
    ...
    function checkInfo() {
        if (pass === "" && fname === "" && lname === "" && email === "" && id === "" && gender !== "" && dqs(`pass`).value === dqs(`confirmPass`).value &&
            ...
            axios.post("http://localhost:5000/lecturer/update", data)
                .then(res => {
                    if (res.data.message === 'Lecturer updated') {
                        deleteCookieAndGoLogin();
                    }
